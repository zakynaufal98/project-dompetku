  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      {isOpen && filtered.length > 0 && (
        <ul className="absolute z-50 w-full mt-2 bg-surface border border-border2 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] shadow-card-md max-h-[200px] overflow-y-auto custom-scrollbar overflow-hidden animate-fade-in transition-colors">
          {filtered.map((d, i) => (
            <li
              key={i}
              onClick={() => {
              ...
              className="px-4 py-3 hover:bg-bg cursor-pointer text-sm font-semibold text-text-2 transition-colors border-b border-border last:border-0"
            >
              {d}
            </li>
          ))}
