...
export default function DescInput({ value, onChange, txData, onEnter }) {
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      <input
        type="text"
        className="form-input w-full bg-white border-slate-200 rounded-xl pl-14 pr-4 py-3 text-sm focus:border-indigo-500 outline-none transition-all"
      ...
      {isOpen && filtered.length > 0 && (
        <ul className="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] max-h-[200px] overflow-y-auto custom-scrollbar overflow-hidden animate-fade-in">
          {filtered.map((d, i) => (
            <li
              key={i}
              onClick={() => {
