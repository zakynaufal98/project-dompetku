...
import { fmt, fmtShort, fmtUnit, today, INV_TYPES } from '../lib/utils'
    ...
    <div className="animate-fade-up space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="bg-surface border border-border rounded-[24px] p-6 lg:p-8 shadow-sm transition-colors">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-border">
          {INV_KEYS.map(k => (
            <div key={k}>
              <p className="text-muted2 text-xs font-semibold flex items-center gap-1.5 mb-1">
                <span className="scale-75 grayscale opacity-70 ">{INV_TYPES[k].icon}</span> {k}
              </p>
              <p className="tabular-nums font-bold text-lg text-text-2">{fmtShort(Math.max(0,byType[k]))}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          return (
            <button key={k} onClick={()=>handleTypeChange(k)}
              className={`flex flex-col items-center justify-center gap-2 py-4 rounded-[20px] border transition-all cursor-pointer ${
                isActive ? 'bg-surface shadow-sm border-border2' : 'bg-bg border-transparent hover:border-border2 opacity-70 hover:opacity-100'
              }`}>
              <div className={`scale-110 transition-all ${isActive ? 'text-income' : 'grayscale text-muted'}`}>{c.icon}</div>
              <span className={`text-xs font-bold tracking-wide ${isActive ? 'text-text' : 'text-muted'}`}>{k}</span>
            </button>
          )
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className={`bg-surface border ${editId ? 'border-primary/50 ring-4 ring-primary/10' : 'border-border'} rounded-[24px] p-6 md:p-8 shadow-sm lg:col-span-3 space-y-5 transition-all`}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-2">
            <button onClick={() => setAction('beli')}
              className={`flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-bold border transition-all cursor-pointer ${
                action === 'beli' ? 'bg-invest-light text-invest border-invest/30' : 'bg-bg text-muted border-transparent hover:border-border2'
              }`}>
              <TrendingUp size={18} strokeWidth={2.5} /> Pembelian
            </button>
            <button onClick={() => setAction('jual')}
              className={`flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-bold border transition-all cursor-pointer ${
                action === 'jual' ? 'bg-gold-light text-gold border-gold/30' : 'bg-bg text-muted border-transparent hover:border-border2'
              }`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Jenis Produk">
              <div className="relative">
                <div className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-bg text-muted rounded-xl flex items-center justify-center pointer-events-none"><Boxes size={16} strokeWidth={2.5} /></div>
            ...
            <Field label="Keterangan">
              <div className="relative">
                <div className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-bg text-muted rounded-xl flex items-center justify-center pointer-events-none"><Pencil size={16} strokeWidth={2.5} /></div>
            ...
            {invType !== 'Uang' && (
              <Field label={cfg.unitLabel}>
                <div className="relative">
                  <div className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-bg text-muted rounded-xl flex items-center justify-center pointer-events-none"><Hash size={16} strokeWidth={2.5} /></div>
                  <input 
                    className={`form-input pl-14 pr-16 py-3 transition-colors ${
                      isQtyError || (isPortfolioEmpty && qty) ? 'border-expense bg-expense-light text-expense focus:border-rose-600' : ''
                    }`} 
                    type="text" 
                    inputMode="decimal" 
                    value={qty} 
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-muted2 font-bold uppercase tracking-wider bg-bg px-2 py-1 rounded-lg">{cfg.unit}</span>
                </div>
                {/* VALIDASI QTY REAL-TIME */}
                {action === 'jual' && isPortfolioEmpty && (
                  <p className="text-[11px] font-bold text-expense mt-1.5 leading-tight">Anda belum memiliki aset {subType} ini.</p>
                )}
                {action === 'jual' && !isPortfolioEmpty && isQtyError && (
                  <p className="text-[11px] font-bold text-expense mt-1.5 leading-tight">Maksimal yang bisa dijual: {fmtUnit(availableQty)} {cfg.unit}</p>
                )}
              </Field>
            )}
            ...
            <Field label="Total Nilai (Rp)">
              <div className="relative">
                <div className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-bg text-muted rounded-xl flex items-center justify-center pointer-events-none"><Banknote size={16} strokeWidth={2.5} /></div>
                <input 
                  className={`form-input pl-14 py-3 transition-colors ${
                    isAmountError || (isPortfolioEmpty && amount && invType === 'Uang') ? 'border-expense bg-expense-light text-expense focus:border-rose-600' : ''
                  }`} 
                  type="text" 
                  inputMode="numeric" 
              ...
              {invType === 'Uang' && action === 'jual' && (
                isPortfolioEmpty ? (
                  <p className="text-[11px] font-bold text-expense mt-1.5 leading-tight">Saldo portofolio {subType} kosong.</p>
                ) : isAmountError ? (
                  <p className="text-[11px] font-bold text-expense mt-1.5 leading-tight">Maksimal tarik: Rp {availableAmount.toLocaleString('id-ID')}</p>
                ) : null
              )}
            </Field>
            ...
            <Field label="Tanggal Transaksi">
              <div className="relative">
                <div className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-bg text-muted rounded-xl flex items-center justify-center pointer-events-none"><CalendarDays size={16} strokeWidth={2.5} /></div>
          ...
          <div className="flex gap-3 mt-2">
            {editId && (
              <button onClick={cancelEdit} className="flex-1 py-4 bg-bg text-text-2 text-sm font-bold rounded-xl hover:bg-border transition-colors">
                Batal Edit
              </button>
            )}
            ...
            {donutData.length > 0 ? (
              <>
                ...
                <div className="space-y-3 mt-6">
                  {donutData.map(d => (
                    <div key={d.name} className="flex items-center justify-between p-2 hover:bg-bg rounded-xl transition-colors">
                      <div className="flex items-center gap-3"><div className="w-3.5 h-3.5 rounded-full" style={{background: d.fill}} /><span className="text-sm font-semibold text-text-2">{d.name}</span></div>
                      <span className="tabular-nums font-bold text-sm text-text">{fmtShort(d.value)}</span>
                    </div>
                  ))}
                </div>
              </>
              ...
              <div className="h-full flex flex-col items-center justify-center opacity-50">
                ...
                <p className="text-sm text-muted2 font-medium">Portofolio kosong</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {Object.keys(subBreakdown).length > 0 && (
          <div className="bg-surface border border-border rounded-[24px] p-6 shadow-sm overflow-x-auto transition-colors">
            ...
            <table className="w-full text-sm mt-2 text-left">
              ...
              <tbody>
                {Object.entries(subBreakdown).map(([sub,v])=>(
                  <tr key={sub} className="border-b border-border hover:bg-bg transition-colors">
                    <td className="py-3 font-bold text-text-2">{sub}</td>
                    <td className="py-3 text-right font-medium text-muted tabular-nums">{v.qty > 0 ? fmtUnit(v.qty) : '—'}</td>
        ...
        <div className="bg-surface border border-border rounded-[24px] p-6 shadow-sm transition-colors">
          ...
          <div className="space-y-1.5 mt-2 lg:max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            {Object.keys(groupedInv).length > 0 ? (
              Object.keys(groupedInv).sort((a,b) => new Date(b) - new Date(a)).map(dateKey => {
                ...
                return (
                  <div key={dateKey} className="mb-6 last:mb-0 animate-fade-up">
                    <div className="flex items-center justify-between border-b-2 border-border pb-2.5 mb-3 sticky top-0 bg-surface z-10">
                      <h4 className={`font-bold text-sm ${isToday ? 'text-invest' : 'text-text-2'}`}>{getDayLabel(dateKey)}</h4>
                      <div className="flex gap-4 text-[11px] font-bold uppercase tracking-wider">
                        {group.totalJual > 0 && <span className="text-gold">+ {fmtShort(group.totalJual)}</span>}
                        {group.totalBeli > 0 && <span className="text-invest">- {fmtShort(group.totalBeli)}</span>}
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      {group.items.map(t => (
                        <div key={t.id} onClick={(e) => { if (e.target.closest('button')) return; handleEditClick(t); }} className="cursor-pointer hover:bg-bg rounded-2xl transition-colors ring-2 ring-transparent hover:ring-emerald-50 hover:ring-invest-light group" title="Klik untuk edit">
              ...
              <div className="py-8"><Empty icon={<BriefcaseBusiness size={40} className="text-muted2 mb-3" strokeWidth={1} />} text="Transaksi tidak ditemukan" /></div>
            )}
          </div>
        </div>
      </div>
